...
@Component({
  ...
  styleUrls: ['./abm-reservation.component.css'],
})
export class AbmReservationComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    this.modalService.loading('Cargando reservas');
    try {
    ...
    if (tableElement) {
      ...
      this.table = new DataTable(tableElement, {
        ...
        data: this.reservations.map((reservation) => [
          reservation.id,
          reservation.workday.date.toString().split('T')[0] +
            ' ' +
            (reservation.appointment?.appointmentTimes.startTime ?? 'N/A'),
          reservation.customer.document,
          reservation.customer.name + ' ' + reservation.customer.lastname,
          reservation.state.name,
          `
          <button class="btn btn-outline-success btn-sm rounded me-2 edit-customer" data-customer='${JSON.stringify(
            reservation
          )}' aria-label="Edit">
  ...
  editReservation(reservation: IReservation): void {
    // Implementa la lógica para editar una reserva
    // this.selectedReservation = reservation;


    
    // this.modalService.formReservation(reservation); // Asumiendo que tienes un modal para editar
  }
  ...
  async cancelReservation(): Promise<void> {
    try {
      if (this.selectedReservation?.state.name === 'earring') {
        ...
        const newCancelReservation =
          await this.gestorCancelarTurnoService.cancelReservation(
            this.selectedReservation.id,
            this.selectedReservation.customer
          );
        ...
        this.table
          ?.row(
            (idx: number, data: any[]) =>
              data[0] === this.selectedReservation!.id
          )
          .data([
            this.selectedReservation.id,
            this.selectedReservation.workday.date.toString().split('T')[0] +
              ' ' +
              (this.selectedReservation.appointment?.appointmentTimes
                ?.startTime ?? 'N/A'),
            this.selectedReservation.customer.document,
            this.selectedReservation.customer.name +
              ' ' +
              this.selectedReservation.customer.lastname,
        ...
        this.modalService.openMenssageTypes({
          ...
          subtitle:
            'La reserva no se puede cancelar porque no está en un estado activo',
          url: null,
          type: 'error',
        });
      }
