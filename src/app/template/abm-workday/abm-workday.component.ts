...
export class AbmWorkdayComponent implements OnInit {
  ...
  previousMonth(): void {
    // cambia el mes y renderiza el calendario nuevamente
    this.currentYear =
  ...
  isDisabledDay(day: number | null): boolean {
    //se encarga de saber si esta desabilitado el dia
    if (day === null) {
      return false;
    } else {
      if (
        ...
        !this.arrayDaysAvaible.includes(day ?? 0)
      ) {
  ...
  isBlockedFuruteDay(day: number | null): boolean {
    //se encarga de saber si esta desabilitado el dia
    if (day === null) {
      return false;
    } else {
  ...
  async getWorkdays(): Promise<void> {
    //consigue los dias
    this.workdays = await this.workdayService.getWorkdays();
  ...
  async selectWorkday(workday: number | null): Promise<void> {
    ...
    const fecha: string = this.formatDateToView(
      this.currentYear,
      this.currentMonth + 1,
      workday ?? 0
    );
